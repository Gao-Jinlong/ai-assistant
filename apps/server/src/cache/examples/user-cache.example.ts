...
export class UserCacheExample {
   ...
   */
  @Cacheable((userId: string) => CacheKeyUtil.userKey(userId, 'posts'), 300)
   ...
   * 示例4：更新数据时清除相关缓存
   */
  @CacheEvict((userId: string) => [
    CacheKeyUtil.userKey(userId, 'profile'),
    CacheKeyUtil.userKey(userId, 'settings'),
    CacheKeyUtil.userKey(userId, 'posts')
  ])
   ...
   * 示例5：批量缓存操作
   */
  async getUsersProfiles(userIds: string[]) {
    // 生成缓存键
   ...
   * 示例6：缓存统计数据
   */
  async getUserStats(userId: string, period: 'daily' | 'weekly' | 'monthly') {
    ...
    return this.cacheService.getOrSet(
      ...
      period === 'daily' ? 3600 : // 1 小时
      period === 'weekly' ? 7200 : // 2 小时  
      14400 // 4 小时（monthly）
    );
  }
  ...
  private async fetchUserSettingsFromDatabase(userId: string): Promise<any> {
    // 模拟数据库查询
    return { userId, theme: 'dark', language: 'zh-CN' };
  }

  private async fetchUserPostsFromDatabase(userId: string): Promise<any[]> {
    // 模拟数据库查询
    return [
  ...
  private async updateUserInDatabase(userId: string, profileData: any): Promise<any> {
    // 模拟数据库更新
  ...
  private async calculateUserStats(userId: string, period: string): Promise<any> {
    // 模拟统计计算
    return {
      userId,
      period,
  ...
  private async performUserSearch(query: string, filters?: any): Promise<any[]> {
    // 模拟搜索操作
    return [
  ...
  private async fetchPaginatedUsers(page: number, limit: number, filters?: any): Promise<any> {
    // 模拟分页查询
    const total = 1000;
